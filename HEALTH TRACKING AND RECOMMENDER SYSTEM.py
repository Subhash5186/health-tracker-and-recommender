...
def BMIRECOMMENDER():
    if(BMI<18.5):
        ...
        print('->Basing meals on potatoes, bread, rice, pasta or other starchy carbohydrates. Choose wholegrain where possible.')
        print('->Choosing unsaturated oils and spreads, such as sunflower or rapeseed, and eating them in small amounts')
        ...
        print("The best way to achieve this is to swap unhealthy and high-energy food choices – such as fast food, processed food and sugary drinks (including alcohol)")
        print("Make PROPER DIET with :")
        print("->plenty of fruit and vegetables")
        print("->plenty of potatoes, bread, rice, pasta and other starchy foods (ideally you should choose wholegrain varieties)")
...
def BPLEVELCALUCLATOR():
    ...
    if(BP_level<=90):
        ...
        print("->High blood pressure is often related to unhealthy lifestyle habits, such as smoking, drinking too much alcohol, being overweight and not exercising enough.")
        print("->Left untreated, high blood pressure can increase your risk of developing a number of serious long-term health conditions, such as coronary heart disease and kidney disease.")

def HEARTBEATLEVELCALUCLATOR():
    global i
    s=Toplevel()
    s.title('HEARTCHECKER')
    i=ImageTk.PhotoImage(Image.open(r"C:\Users\Damarla Sankar\Downloads\HEARTRATECHECK.png"))
...
def HEALTHQUOTE():
    ...
    j=ImageTk.PhotoImage(Image.open(r"C:\Users\Damarla Sankar\Downloads\h2.jpg"))
...
def RECOMMENDER():
    ...
    f1.pack(side=TOP)
    lb1_f1 = Label(f1, text=" HEALTH RECOMMENDER SYSTEM", padx=16, pady=16, bd=16, fg='#000000',font=("Bell MT", 48, 'bold'), bg="#FFBFB9", relief='raise', width=28, height=1)
    lb1_f1.pack()
    
    #this is for Getting DIET as per BMI
    BMI_INDEX=Button(k,text='BMI DIET RECOMMENDER :',command=BMIRECOMMENDER, bg="navajo white").pack(padx=15,pady=5,anchor=CENTER)
    #this is for results of entered BP levels
    BP_INDEX=Button(k,text='CHECK BP LEVELS :',command=BPLEVELCALUCLATOR, bg="navajo white").pack(padx=15,pady=5,anchor=CENTER)

#this is for MAIN HEADING
Tops = Frame(tk, width=1350, height=50, bd=8, relief="raise")
Tops.pack(side=TOP)
lbl_T1 = Label(Tops, text="HEALTH TRACKING SYSTEM", padx=16, pady=16, bd=16, fg='#000000',font=("Algerian", 48, 'bold'), bg="navajo white", relief='raise', width=28, height=1)
lbl_T1.pack()
...
#this is for FEMALE
Radiobutton(tk,text='Female',variable=gender_var,value='Female', bg="navajo white").pack(padx=15,pady=5,anchor=CENTER)
...
HEARTBEAT_INDEX=Button(tk,text='CHECK HEART BEAT LEVELS :',command=HEARTBEATLEVELCALUCLATOR, bg="navajo white").pack(padx=15,pady=5,anchor=CENTER)

#this is also a TOPLEVEL to get DIET and BP
HEALTH_RECOMMENDER=Button(tk,text='HEALTH RECOMMENDER',command=RECOMMENDER, bg="navajo white").pack(padx=15,pady=5,anchor=CENTER)
